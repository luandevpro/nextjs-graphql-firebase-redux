...
function getCookie(context = {}) {
  return context.req && context.req.cookies.token ? context.req.cookies.token : globalToken;
}
...
export default (App, { loginRequired = true, logoutRequired = false } = {}) => {
  ...
  Apollo.getInitialProps = async (ctx) => {
    ...
    if (ctx.req && ctx.req.cookies.token) {
      try {
        ...
        if (data.users[0]) {
          ...
          const userDecode = decode(ctx.req.cookies.token);
    ...
    const token = ctx.req && ctx.req.cookies.token ? ctx.req.cookies.token : globalToken;
