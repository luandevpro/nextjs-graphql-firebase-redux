...
export default (App, { loginRequired = true, logoutRequired = false } = {}) => {
  function Apollo({ apolloState, ...props }) {
    ...
    useEffect(() => {
      const { user, isFromServer, token } = props;
      if (isFromServer) {
        globalUser = user;
        globalToken = token;
      }
  ...
  Apollo.getInitialProps = async ({ store, ...ctx }) => {
    ...
    if (ctx.req && ctx.req.signedCookies.token) {
      try {
        ...
        store.dispatch(actions.getCurrentUser(data.users[0]));
      } catch (e) {
        console.log(e);
      }
    } else {
