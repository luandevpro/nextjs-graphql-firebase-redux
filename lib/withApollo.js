...
export default (App, { ssr = true, loginRequired = true, logoutRequired = false } = {}) => {
  ...
  if (ssr || (App.getInitialProps && App.displayName !== 'MyApp')) {
    Apollo.getInitialProps = async (ctx) => {
      ...
      if (ctx.req && ctx.req.signedCookies.token) {
        ...
        try {
          ...
          if (data.users_by_pk) {
            ctx.store.dispatch(actions.getCurrentUser(data.users_by_pk));
